...
public class BTree {
	...
	class Node {
		...
		void insert(Albume[] s) {
			for (int i = 0; i < order - 1; i++)
				if (s[i] != null) insert(s[i]);
		}

		boolean rid(Albume s) {
			if (!contains(s)) return false;
    	for (int i = 0; i < order-1; i++)
	...
	Node parent(Node h, Albume s, Node parent) {
	...
	//adds value to btree
	boolean add(Node h, Albume s, Node parent) {
		if (h.isFull())
			if (h == root) {
				h.splitRoot();
				return add(s);
	...
	boolean remove(Node h, Albume s, Node parent) {
	...
	public boolean remove(Albume s) {
	...
	boolean search(Node h, Albume s, Node parent) {
	...
	public boolean search(Albume s) {
