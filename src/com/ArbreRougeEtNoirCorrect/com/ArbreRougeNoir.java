...
public class ArbreRougeNoir {
    ...
    public Noeud rechercher(String titre) {
        return rechercher(getRacine(), new Data(titre, "", "", "", ""));
    }
    ...
    private Noeud rechercher(Noeud n, Data o) {
        ...
        if (o.compareTo(n.getInfo()) < 0) {
            return rechercher(n.getGauche(), o);
        } else if (o.compareTo(n.getInfo()) == 0) {
        	JOptionPane.showMessageDialog(null, "Album Existant :) ", null, JOptionPane.INFORMATION_MESSAGE);
            return n;
        } else {
    ...
    private Noeud ajout(Data o, Noeud r, Noeud p) {
        ...
        if (r.isSentinelle()) {
        	...
        	{
        		JOptionPane.showMessageDialog(null, "Deja Existant !!", null, JOptionPane.ERROR_MESSAGE);
        	}
        	else
        	{
    ...
    private Noeud supprimer(Noeud r, Data o) {
        if (r.isSentinelle()) {
        	JOptionPane.showMessageDialog(null, "Pas Trouve", null, JOptionPane.ERROR_MESSAGE);
            return r; // Pas trouvé
        }
        if (o.compareTo(r.info) == 0) {
        	JOptionPane.showMessageDialog(null, "Supprimer Avec Succes", null, JOptionPane.INFORMATION_MESSAGE);
            r = supprimer(r);
        } else if (o.compareTo(r.info) < 0) {
   ...
   // le noeud successseur de x dans l'arbre,
    ...
    public void afficherPrefixe(Noeud racine, JTextArea textArea)
    {
    	if(racine != sentinelle)
    	{
    		if(racine.getCouleur() == Color.BLACK)
    		{
    			textArea.setText(textArea.getText() + racine.getInfo().getTitre() + " ------------------> " + "Noir" + "\n\n");
    		}
    		else
    		{
    			textArea.setText(textArea.getText() + racine.getInfo().getTitre() + " ------------------> " + "Rouge" + "\n\n");
    		}
    	...
    	{
    		afficherPrefixe(racine.getGauche(), textArea);
    	}
    	
    	if(racine.getDroit() != null)
    	{
    		afficherPrefixe(racine.getDroit(), textArea);
    	}
    }
}
